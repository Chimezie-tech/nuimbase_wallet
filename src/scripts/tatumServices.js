...
const COINGECKO_MAP = {
  ...
  ONE: 'harmony',
  LTC: 'litecoin',
  CELO: 'celo', // ... add other crypto assets as needed ...
}
...
export const getFiatRateUSD = async (fiat) => {
  ...
  try {
    ...
    const data = await res.json() // The result format is { "rates": { "EUR": 0.92 } }
    return data.rates?.[fiat.toUpperCase()] || 1
  } catch (e) {
    console.error('Error fetching fiat rate:', e.message) // Fallback: default to 1 so the crypto USD price is used directly (if fiat is not found)
    return 1
  }
}
...
async getUTXOs(chain, address, amountBtc = 0) {
  if (!['BTC', 'LTC'].includes(chain.toUpperCase())) return [];

  try {
    const isTestnet = this.isTestnet();
    const chainName = chain.toLowerCase() === 'btc' ? 'bitcoin' : 'litecoin';
    const network = isTestnet ? `${chainName}-testnet` : chainName;

    // 1. Convert BTC to Satoshis and ensure it's an INTEGER
    // We multiply by 10^8 and use Math.round to kill the decimals
    const satoshisNeeded = Math.round(parseFloat(amountBtc || 0.0001) * 1e8);

    const url = new URL(`https://api.tatum.io/v4/data/utxos`);
    url.searchParams.append('chain', network);
    url.searchParams.append('address', address);
    url.searchParams.append('totalValue', satoshisNeeded.toString());

    const res = await fetch(url.toString(), { headers: getHeaders() });

    if (!res.ok) {
      console.warn(`UTXO API returned ${res.status}. Wallet might be empty.`);
      return [];
    }

    // Inside getUTXOs function
      const data = await res.json();

      return Array.isArray(data) ? data.map(u => {
        // ⭐ THE FIX: Convert BTC decimal to Satoshi whole number
        // If the value is 0.00032303, this makes it 32303
        const satoshiValue = Math.round(parseFloat(u.value) * 100000000);

        return {
          txHash: u.txHash || u.txid, // Handle both Tatum and Blockstream keys
          index: u.index !== undefined ? u.index : u.vout,
          value: BigInt(satoshiValue), // Now BigInt will be happy
          address: address
        };
      }) : [];
