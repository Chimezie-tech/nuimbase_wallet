  ...
  <div class="send-wrapper">
    ...
    <div class="fab-container">
      ...
      <button class="main-fab" :class="{ 'is-open': isFabOpen }" @click="isFabOpen = !isFabOpen">
        <i
          class="pi pi-plus transition-transform duration-300"
    ...
    >
      <form @submit.prevent="onReview" v-if="step === 1" class="flex flex-col gap-3">
        ...
        <div
          class="flex justify-between items-center bg-gray-50 px-2 py-1.5 rounded border border-gray-100"
        >
          ...
          <div class="text-[11px] font-semibold text-gray-700">
            ...
            <span class="text-gray-400 font-normal"
              >({{ formatFiat(currentBalance * fiatForm.rate) }})</span
            >
          </div>
        </div>
        ...
        <div class="flex flex-col gap-1">
          ...
          <small class="text-red-500 text-[9px] leading-tight" v-if="validationError">{{
            validationError
          }}</small>
        </div>
      ...
      <form @submit.prevent="onSend" v-if="step === 2" class="flex flex-col gap-3">
          ...
          <div class="flex justify-between items-end mb-2">
            ...
            <div class="text-right">
              <strong class="block text-sm text-gray-800"
                >{{ stForm.amount }} {{ getCurrencyLabel() }}</strong
              >
        ...
        <div class="flex flex-col gap-1">
          <label class="text-[10px] font-bold text-gray-500 uppercase text-center"
            >Confirm PIN</label
          >
        ...
        <div class="flex gap-2 mt-1">
          ...
          <Button
            type="submit"
            label="Confirm Send"
            class="flex-1 !py-1 !text-xs btn-brand"
            :loading="isSending"
    ...
    >
      <div id="receipt-content" class="p-6 text-center">
        ...
        <p v-if="!txResult.success" class="text-sm text-gray-500 mt-2 px-2">
          {{ txResult.errorMessage }}
        </p>

        <div
          v-if="txResult.success"
          class="mt-6 text-left bg-gray-50 p-4 rounded-lg border border-gray-100"
        >
          <div class="result-row py-2">
            <span class="text-xs text-gray-500">Amount</span>
            <strong class="text-sm text-gray-800"
              >{{ txResult.amount }} {{ txResult.blockchain }}</strong
            >
          </div>
          <div class="result-row py-2">
          ...
          <div class="result-row border-0 py-2">
            ...
            <div class="flex items-center gap-2">
              <span class="text-truncate font-mono text-xs" style="max-width: 140px">{{
                txResult.txHash
              }}</span>
              <i
                class="pi pi-copy text-xs cursor-pointer text-brand hover:text-green-700"
                @click="copyText(txResult.txHash)"
              ></i>
            </div>
          </div>
        </div>
        ...
        <div class="mt-8 flex flex-col sm:flex-row gap-3">
          ...
          <Button
            label="Close"
            class="flex-1 !py-2.5 !text-sm btn-brand"
            @click="keys.modalResult = false"
          />
        </div>
    ...
    >
      <div id="printable-receive" class="flex flex-col items-center gap-3 text-center">
        ...
        <div class="w-full bg-gray-50 p-2 rounded border border-gray-200">
          ...
          <p class="text-[10px] font-mono text-gray-700 break-all leading-tight">
            {{ receiveForm.address }}
          </p>
        </div>
      </div>
    ...
    <Dialog
      v-model:visible="keys.scannerOpen"
      header="Scan QR"
      modal
      class="compact-dialog"
      :style="{ width: '310px' }"
    ...
    <div v-if="toast.show" :class="['toast', toast.type]">
      <i
        :class="toast.type === 'success' ? 'pi pi-check' : 'pi pi-info-circle'"
...
const tokens = [
  ...
  { label: 'USDT (Ethereum)', key: 'USDT_ETH', blockchain: 'ETH', isToken: true, symbol: 'USDT' },
  {
    label: 'USDT (Binance Smart Chain)',
    key: 'USDT_BSC',
    blockchain: 'BSC',
    isToken: true,
    symbol: 'USDT',
  },
  { label: 'USDC (Ethereum)', key: 'USDC_ETH', blockchain: 'ETH', isToken: true, symbol: 'USDC' },
  { label: 'USDT (Solana)', key: 'USDT_SOL', blockchain: 'SOLANA', isToken: true, symbol: 'USDT' },
]
...
const getCurrentAsset = () =>
  tokens.find((t) => t.key === stForm.value.selectedAssetKey) || tokens[0]
const getReceiveAsset = () =>
  tokens.find((t) => t.key === receiveForm.value.selectedAssetKey) || tokens[0]
    ...
    if (res && res.wallets) {
      ...
      userWallets.value.forEach(async (w) => {
        ...
        if (balRes.success) {
          ...
          if (idx !== -1)
            userWallets.value[idx].balance = balRes.balance.availBalance || balRes.balance.incoming
        }
      })
    }
    updateExchangeRate()
...
const onSend = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      txResult.errorMessage =
        res.error ||
        'The blockchain network rejected this transaction. Please check your balance and try again.'
    }
    ...
    txResult.errorMessage =
      'Network timeout. Please check your transaction history in a few minutes.'
    keys.modalResult = true
  } finally {
...
const generateReceiveQR = async () => {
  ...
  if (wallet && wallet.address) {
    ...
    console.log(
      'Available Wallets:',
      userWallets.value.map((w) => w.blockchain),
    )
...
const downloadReceivePDF = () => {
  ...
  printWindow.document.write(
    '<html><head><title>Receive</title><style>body{text-align:center; font-family:sans-serif; padding-top:50px;}</style></head><body>',
  )
